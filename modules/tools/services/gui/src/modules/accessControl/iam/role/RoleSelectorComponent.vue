<template>
    <div class="row q-pa-md" style="max-width: 95%">
      ...
      <div class="col-12 q-mt-md row">
        <q-btn color="negative" class="col-2" @click="emit('canceled')">{{ $t('modules.accessControl.iam.role.select.cancelButton') }}</q-btn>
        <div class="col-5"></div>
        <q-btn color="positive" class="col-5" :disable="selectedUUID == ''" @click="emit('selected', props.namespace, selectedUUID)">{{ $t('modules.accessControl.iam.role.select.selectButton') }}</q-btn>
      </div>
    </div>
</template>
