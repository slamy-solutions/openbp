...
/**
 * @group native/actor/user/getByLogin/whitebox
 * @group whitebox
 */
describe("Whitebox", () => {
    test("Gets data from cache if cache enabled. Doesnt get data from cache if cache disabled", async () => {
        ...
        await nativeActorUserGRPC.GetByLogin({
            useCache: true,
            login
        })
        ...
        const cachedResponse = await nativeActorUserGRPC.GetByLogin({
            useCache: true,
            login
        })
        expect(cachedResponse.user?.avatar).toBe(avatar)

        const notCachedResponse = await nativeActorUserGRPC.GetByLogin({
            useCache: false,
            login
        })
...
/**
 * @group native/actor/user/getByLogin/blackbox
 * @group blackbox
 */
describe("Blackbox", () => {
    test("Gets actual user data", async () => {
        ...
        const getResponse = await nativeActorUserGRPC.GetByLogin({
            useCache: false,
            login
        })
    ...
    test("Cached response gets actual user data", async () => {
        ...
        await nativeActorUserGRPC.GetByLogin({ useCache: true, login })
        const getResponse = await nativeActorUserGRPC.GetByLogin({
            useCache: true,
            login
        })
    ...
    test("Fails with NOT_FOUND error if user with specified login doesnt exist",  async () => { 
        try {
            await nativeActorUserGRPC.GetByLogin({ useCache: false, login: randomBytes(16).toString('hex')})
            fail()
        } catch (e) {
