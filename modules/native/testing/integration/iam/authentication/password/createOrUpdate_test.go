...
func (s *CreateOrUpdateTestSuite) TestCreateAndUpdateInGlobalNamespace() {
	...
	passwordResponse, err := s.nativeStub.Services.IAM.Authentication.Password.CreateOrUpdate(ctx, &password.CreateOrUpdateRequest{
		Namespace: "",
	...
	authResponse, err := s.nativeStub.Services.IAM.Authentication.Password.Authenticate(ctx, &password.AuthenticateRequest{
		Namespace: "",
		Identity:  identityCreateResponse.Identity.Uuid,
		Password:  pwd1,
	})
