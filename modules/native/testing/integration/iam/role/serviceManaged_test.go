...
func (s *ServiceManagedTestSuite) TestServiceManagedForGlobalNamespace() {
	...
	getResponse, err := s.nativeStub.Services.IAM.Role.GetServiceManagedRole(ctx, &role.GetServiceManagedRoleRequest{
		Namespace: "",
		Service:   managedService,
		ManagedId: managedId,
		UseCache:  true,
...
func (s *ServiceManagedTestSuite) TestServiceManagedFailsWithAlreadyExistForSameMnagementIdForGlobalNamespace() {
	...
	createResponse2, err := s.nativeStub.Services.IAM.Role.Create(ctx, &role.CreateRoleRequest{
		Namespace:   "",
		Name:        tools.GetRandomString(20),
		Description: "",
		Managed: &role.CreateRoleRequest_Service{Service: &role.ServiceManagedData{
	...
	if err == nil {
		s.nativeStub.Services.IAM.Role.Delete(context.Background(), &role.DeleteRoleRequest{Namespace: "", Uuid: createResponse2.Role.Uuid})
...
func (s *ServiceManagedTestSuite) TestServiceManagedFailWithNotFoundForBabManagedIdForGlobalNamespace() {
	...
	_, err = s.nativeStub.Services.IAM.Role.GetServiceManagedRole(ctx, &role.GetServiceManagedRoleRequest{
		Namespace: "",
		Service:   managedService,
		ManagedId: tools.GetRandomString(20),
		UseCache:  true,
