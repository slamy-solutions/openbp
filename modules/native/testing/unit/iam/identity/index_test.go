...
type IndexIdentityTestSuite struct {
	suite.Suite
...
func (suite *IndexIdentityTestSuite) SetupSuite() {
...
func (suite *IndexIdentityTestSuite) TearDownSuite() {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*30)
	defer cancel()
...
func TestIndexIdentityTestSuite(t *testing.T) {
	suite.Run(t, new(IndexIdentityTestSuite))
}

func (s *IndexIdentityTestSuite) TestIndexCreationOnNewNamespace() {
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*30)
	defer cancel()
	...
	for time.Now().UTC().Unix()-waitStartTimestamp < 20 {
		specs, err := s.systemStub.DB.Database("openbp_namespace_" + namespaceName).Collection("native_iam_identity").Indexes().ListSpecifications(ctx)
		require.Nil(s.T(), err)

		for _, index := range specs {
			if index.Name == "fast_search_service" {
				return
			}
		}
