...
const (
	NAMESPACE_CREATION_EVENT_CONSUMER_NAME = "native_iam_policy_namespacecreation"
)
...
func NewEventHandlerService(systemStub *system.SystemStub) (*eventHandlerService, error) {
	...
	_, err = js.AddConsumer("native_namespace_event", &nats.ConsumerConfig{
		...
		Description:    "Listens on native_namespace create events for native_iam_policy",
		AckPolicy:      nats.AckExplicitPolicy,
		FilterSubject:  "native.namespace.event.created",
		DeliverSubject: "native.iam.policy.deliver.namespace.create",
		DeliverGroup:   "native.iam.policy.deliver.namespace.create",
	})
	if err != nil {
		return nil, errors.New("Error while creating consumer. " + err.Error())
	}
	subscribtion, err := js.QueueSubscribe("native.namespace.event.created", "native.iam.policy.deliver.namespace.create", service.handleNamespaceCreationEvent, nats.Bind("native_namespace_event", NAMESPACE_CREATION_EVENT_CONSUMER_NAME))
	if err != nil {
...
func (s *eventHandlerService) handleNamespaceCreationEvent(msg *nats.Msg) {
	...
	err = ensureBuiltInsForNamespace(ctx, namespace.Name, s.systemStub)
	if err != nil {
