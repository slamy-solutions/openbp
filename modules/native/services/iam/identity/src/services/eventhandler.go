...
const (
	NAMESPACE_CREATION_EVENT_CONSUMER_NAME = "native_iam_identity_namespacecreation"
)
...
func NewEventHandlerService(systemStub *system.SystemStub) (*eventHandlerService, error) {
	...
	_, err = js.AddConsumer("native_namespace_event", &nats.ConsumerConfig{
		...
		Description:    "Listens on native_namespace create events for native_iam_identity",
		AckPolicy:      nats.AckExplicitPolicy,
		FilterSubject:  "native.namespace.event.created",
		DeliverSubject: "native.iam.identity.deliver.namespace.create",
		DeliverGroup:   "native.iam.identity.deliver.namespace.create",
	})
	if err != nil {
		return nil, errors.New("Error while creating consumer. " + err.Error())
	}
	subscribtion, err := js.QueueSubscribe("native.namespace.event.created", "native.iam.identity.deliver.namespace.create", service.handleNamespaceCreationEvent, nats.Bind("native_namespace_event", NAMESPACE_CREATION_EVENT_CONSUMER_NAME))
	if err != nil {
