...
func main() {
	systemStub := system.NewSystemStub(
		system.NewSystemStubConfig().WithCache().WithDB().WithVault().WithOTel(system.NewOTelConfig("native", "iam_token", VERSION, getHostname())),
	)
	...
	iamTokenServer := services.NewIAmTokenServer(systemStub, nativeStub)
