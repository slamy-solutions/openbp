...
type ListDevicesTestSuite struct {
	suite.Suite
...
func (suite *ListDevicesTestSuite) SetupSuite() {
...
func (suite *ListDevicesTestSuite) TearDownSuite() {
	suite.nativeStub.Close()
}
func TestListDevicesTestSuite(t *testing.T) {
...
func (s *CountTestSuite) TestListDevices() {
	...
	require.Nil(s.T(), err)
	listedDevices := make([]string, 0, 10)
	for {
		...
		listedDevices = append(listedDevices, d.Device.Device.Uuid)
	}
	require.Equal(s.T(), devices[4*devicePerFleetCount:4*devicePerFleetCount+4], listedDevices)
	...
	require.Equal(s.T(), devices[4*devicePerFleetCount+1:4*devicePerFleetCount+4], listedDevices)
